...
export default function MySupportPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'pending':
        return (
          <span className="px-3 py-1 bg-yellow-100/60 text-yellow-700 rounded-lg text-md font-medium">
            <FontAwesomeIcon icon={faHourglassStart} /> В ожидании
          </span>
        );
      case 'answered':
        return (
          <span className="px-3 py-1 bg-green-100/60 text-green-700 rounded-lg text-md font-medium">
            <FontAwesomeIcon icon={faCheck} /> Отвечено
          </span>
        );
      case 'closed':
        return (
          <span className="px-3 py-1 bg-gray-100/60 text-gray-700 rounded-lg text-md font-medium">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-4">
              ...
              <Link
                href="/support"
                className="bg-linear-to-r from-[#FF7340] to-[#FF4500] hover:opacity-80 text-white font-semibold py-3 px-6 rounded-lg transition-all duration-200"
              >
                + Новое обращение
              </Link>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-white rounded-2xl shadow-sm border p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-gray-500">Всего обращений</p>
                <p className="text-3xl font-bold text-gray-900">{totalCount}</p>
              </div>
              <div className="w-12 h-12 rounded-full bg-blue-100 flex items-center justify-center">
                <span className="text-2xl"><FontAwesomeIcon icon={faEnvelope} /></span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-gray-500">Ожидают ответа</p>
                <p className="text-3xl font-bold text-gray-900">
                  {tickets.filter(t => t.status === 'pending').length}
                </p>
              </div>
              <div className="w-12 h-12 rounded-full bg-yellow-100 flex items-center justify-center">
                <span className="text-2xl"><FontAwesomeIcon icon={faHourglassStart} /></span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-gray-500">Получено ответов</p>
                <p className="text-3xl font-bold text-gray-900">
                  {tickets.filter(t => t.status === 'answered').length}
                </p>
              </div>
              <div className="w-12 h-12 rounded-full bg-green-100 flex items-center justify-center">
                <span className="text-2xl"><FontAwesomeIcon icon={faCheck} /></span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="mb-6">
          <div className="flex flex-wrap gap-2">
            <button
              ...
              className={`px-4 py-2 rounded-lg transition-all ${filterStatus === 'all' ? 'bg-[#FF7340] text-white' : 'bg-white text-gray-700 hover:bg-gray-50 border'} cursor-pointer`}
            >
              Все
            </button>
            <button
              onClick={() => setFilterStatus('pending')}
              className={`px-4 py-2 rounded-lg transition-all ${filterStatus === 'pending' ? 'bg-[#FF7340] text-white' : 'bg-white text-gray-700 hover:bg-gray-50 border'} cursor-pointer`}
            >
              В ожидании
            </button>
            <button
              onClick={() => setFilterStatus('answered')}
              className={`px-4 py-2 rounded-lg transition-all ${filterStatus === 'answered' ? 'bg-[#FF7340] text-white' : 'bg-white text-gray-700 hover:bg-gray-50 border'} cursor-pointer`}
            >
              Отвечено
            </button>
            <button
              onClick={() => setFilterStatus('closed')}
              className={`px-4 py-2 rounded-lg transition-all ${filterStatus === 'closed' ? 'bg-[#FF7340] text-white' : 'bg-white text-gray-700 hover:bg-gray-50 border'} cursor-pointer`}
            >
              Закрыто
            </button>
          </div>
        ...
        <div className="bg-white rounded-2xl border overflow-hidden">
          {isLoading ? (
            <div className="py-12 text-center flex items-center justify-center gap-2">
              ...
              <p className="text-gray-600">Загрузка ваших обращений...</p>
            </div>
          ) : tickets.length === 0 ? (
            <div className="py-16 text-center">
              <div className="text-6xl mb-4"><FontAwesomeIcon icon={faInbox} /></div>
              <h3 className="text-xl font-bold text-gray-900 mb-2">
                У вас пока нет обращений
              </h3>
              <p className="text-gray-600 mb-6 max-w-md mx-auto">
                {filterStatus !== 'all' 
                  ? `Нет обращений со статусом "${filterStatus === 'pending' ? 'В ожидании' : filterStatus === 'answered' ? 'Отвечено' : 'Закрыто'}"`
                  : 'Обратитесь в поддержку, если у вас есть вопросы или проблемы'}
              </p>
              <Link
                href="/support"
                className="inline-block bg-linear-to-r from-[#FF7340] to-[#FF4500] text-white px-6 py-3 rounded-lg hover:opacity-80"
              >
                    ...
                    <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 mb-4">
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-4 text-gray-500">
                          <span><FontAwesomeIcon icon={faCalendarDays} className="text-md" /> <span className="text-sm">{formatDate(ticket.createdAt)}</span></span>
                          {ticket.answeredAt && (
                            <span><FontAwesomeIcon icon={faCheck} /> Ответ: {formatDate(ticket.answeredAt)}</span>
                          )}
                        </div>
                      </div>
                        ...
                        <div className="flex gap-2">
                          {ticket.answer && !ticket.isReadByUser && (
                            <button
                              ...
                              className="px-4 py-2 bg-[#FF7340] hover:bg-[#FF4500] text-white rounded-lg text-sm font-medium transition-colors"
                            >
                      ...
                      <div className="mt-4 pt-4 border-t border-gray-200">
                        <div className="flex items-center justify-between mb-3">
                          <h4 className="font-bold text-gray-900 flex items-center gap-2">
                            <span className="text-[#FF7340] text-md"><FontAwesomeIcon icon={faCommentDots} /></span>
                            Ответ службы поддержки
                          </h4>
                          {!ticket.isReadByUser && (
                            <button
                              onClick={() => handleMarkAsRead(ticket.id)}
                              className="text-sm text-[#FF7340] hover:text-[#FF4500] font-medium"
                            >
                              Отметить как прочитанное
                            </button>
                          )}
                        ...
                        <div className="bg-gray-50 rounded-xl p-4">
                          <div className="flex items-start gap-3 mb-3">
                            <div className="w-10 h-10 rounded-full bg-[#FF7340]/20 flex items-center justify-center">
                              <span className="text-[#FF7340] font-bold">A</span>
                            </div>
                            <div>
                              <p className="font-medium text-gray-900">Администратор</p>
                              <p className="text-sm text-gray-500">Служба поддержки сайта Сокровища Народов</p>
                            </div>
                          </div>
                          
                          <div className="pl-13">
                            <p className="text-gray-700 whitespace-pre-line">{ticket.answer}</p>
                            {ticket.answeredAt && (
                        ...
                        {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                          ...
                          return (
                            <button
                              ...
                              className={`w-10 h-10 flex items-center justify-center rounded-lg mx-1 ${currentPage === pageNum ? 'bg-[#FF7340] text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
                            >
                              {pageNum}
                            </button>
                          );
        ...
        <div className="mt-8 bg-white rounded-2xl shadow-sm border p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <h4 className="font-medium text-gray-900 mb-2">Сколько ждать ответа?</h4>
              <p className="text-gray-600 text-sm">
                Мы отвечаем на обращения в течение 24 часов в рабочие дни (пн-пт с 9:00 до 18:00).
              </p>
            </div>
            <div>
              <h4 className="font-medium text-gray-900 mb-2">Как закрыть обращение?</h4>
              <p className="text-gray-600 text-sm">
                После получения ответа вы можете закрыть обращение, нажав кнопку "Закрыть обращение".
              </p>
            </div>
            <div>
              <h4 className="font-medium text-gray-900 mb-2">Не пришел ответ?</h4>
              <p className="text-gray-600 text-sm">
                Проверьте папку "Спам" в вашей почте. Также ответ всегда доступен на этой странице.
              </p>
            </div>
            <div>
              <h4 className="font-medium text-gray-900 mb-2">Срочный вопрос?</h4>
              <p className="text-gray-600 text-sm">
